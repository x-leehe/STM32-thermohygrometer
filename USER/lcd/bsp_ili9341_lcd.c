...
void LCD_DrawPicture(uint8_t * pic_data)
{
	uint32_t i = 0;
	uint32_t pixel_len;
	uint32_t pixel;
	
	pixel_len = 2*((uint32_t)(pic_data[3] << 8) + pic_data[2])*((uint32_t)(pic_data[5] << 8) + pic_data[4]);
	
	ILI9341_Write_Cmd ( CMD_SetPixel );	
	
	while( i < pixel_len)
	{
		pixel = (uint32_t)(pic_data[9+i] << 8) + pic_data[8+i];
		ILI9341_Write_Data(pixel);
		i+=2;
	}
